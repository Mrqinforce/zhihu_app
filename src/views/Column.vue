<template>
	<div>
		<div class="nav">
			<svg viewBox="0 0 200 91" class="ZhihuLogo" style="height:30px;width:64px; margin-top:	20px;" width="64" height="30" aria-hidden="true">
				<title></title>
				<g>
					<path
		...
		<div>
			...
			<a target="_blank" type="button" class="Button ColumnHomeTop-writeButton Button--grey" href="/write">开始写文章</a>
			<a target="_blank" type="button" class="Button ColumnHomeTop-requestButton Button--plain Button--green" href="https://www.zhihu.com/column/request">
				申请开通专栏
				<span style="display:inline-flex;align-items:center">
					​
					<svg class="Zi Zi--ArrowRight" fill="currentColor" viewBox="0 0 24 24" width="1.2em" height="1.2em">
						<path
		...
		<div class="backGround"><img src="../assets/img/background.png" /></div>
		<div class="column_find"><p class="zhuanlan">专栏 · 发现</p></div>
		<div class="column">
			<div class="column_one" v-for="(item, index) in columns" :key="index">
				<img :src="item.imageUrl" />
				<span class="column_title">{{ item.title.slice(0, 6) }}</span>
				<span class="column_description">{{ item.description.slice(0, 10) }}</span>
				<p class="column_folllow">{{ item.followers }}关注 | {{ item.articlesCount }}文章</p>
				<div>
...
export default {
	...
	created() {
		this.axios({
			method: 'post',
			url: 'http://localhost:8080/api/column/page',
			params: {
				count: this.count,
